...
const dashboardPeriods = {
  today: {deposit:12450, payout:8920, profit:3530, open:42},
  days3: {deposit:39760, payout:26340, profit:13420, open:96},
  week: {deposit:92430, payout:61870, profit:30560, open:188},
  month: {deposit:368161, payout:244748, profit:123413, open:512}
};
...
function pageHead(icon,title,sub,action=''){ return `<div class="page-head"><div class="page-icon">${icon}</div><div><h1>${title}</h1><p>${sub}</p></div>${action?`<div class="head-action">${action}</div>`:''}</div>`; }
...
function footer(){ return `<footer class="footer"><span>© 2024 BetXsoft. Alle Rechte vorbehalten.</span><span>Version 1.0.0</span><i class="mobile-home-indicator"></i></footer>`; }
function header(){ return `<header class="topbar"><div class="mobile-status"><strong>23:06</strong><span class="status-icons"><i class="cell-bars"><b></b><b></b><b></b><b></b></i><i class="wifi-mark"></i><i class="battery-mark"><b>39</b></i></span></div><div class="topbar-main"><div class="brand-zone"><button class="brand" data-go="home" aria-label="Startseite">Bet<span class="brand-x">X</span>soft<small>Shop Admin UI</small></button><div class="mobile-total"><small>Gesamtbalance</small><strong>368.161,00</strong></div></div><button class="home-button" data-go="home" aria-label="Startseite">⌂</button><div class="top-spacer"></div><div class="balance"><span class="balance-icon">${svgIcon('wallet')}</span><span class="balance-copy"><small>Guthaben</small><strong>14.857,00</strong></span></div><button class="menu-button" data-drawer aria-label="Menü öffnen">${svgIcon('menu')}</button></div></header>`; }
...
function payout2(){
  ...
  return `${pageHead(svgIcon('up'),'Auszahlung','',payoutClose())} ${steps(2,'Auszahlung')}<section class="card card-pad flow-card"><h2>Auszahlung bestätigen</h2><p class="muted">Bitte überprüfen Sie Ihre Angaben vor der Bestätigung.</p><div class="summary-list"><div class="summary-row"><span>Benutzer-ID</span><strong>${esc(state.customer)}</strong></div><div class="summary-row payout-amount-highlight"><span>Betrag</span><strong class="negative">${formatAccountBalance(amount)}</strong></div><div class="summary-row"><span>Aktuelles Guthaben</span><strong>${formatAccountBalance(currentBalance)}</strong></div><div class="summary-row"><span>Neues Guthaben</span><strong>${formatAccountBalance(newBalance)}</strong></div></div><div class="actions"><button class="btn secondary" data-go="payout-1">Zurück</button><button class="btn danger" data-confirm-payout>Auszahlung bestätigen</button></div></section>`; }

function payout3(){
  const amount=Number(state.amount||'0');
  const newBalance=customerBalanceValue(state.customer);
  return `${pageHead(svgIcon('up'),'Auszahlung','',payoutClose())} ${steps(3,'Auszahlung')}<section class="card flow-card success-panel"><div class="success-mark payout-success-mark">✓</div><h2>Auszahlung erfolgreich!</h2><p class="muted">Die Auszahlung wurde erfolgreich durchgeführt.</p><div class="summary-list"><div class="summary-row"><span>Neues Guthaben</span><strong>${formatAccountBalance(newBalance)}</strong></div><div class="summary-row"><span>Benutzer-ID</span><strong>${esc(state.customer)}</strong></div><div class="summary-row"><span>Betrag</span><strong class="negative">${formatAccountBalance(amount)}</strong></div></div><div class="actions"><button class="btn block" data-go="home">Zurück zur Übersicht</button></div></section>`;
}
...
function bind(){
  ...
  document.querySelector('[data-range-apply]')?.addEventListener('click',()=>{const from=document.querySelector('[data-range-from]').value;const to=document.querySelector('[data-range-to]').value;const start=new Date(from+'T00:00:00');const end=new Date(to+'T00:00:00');if(!from||!to||end<start)return toast('Bitte einen gültigen Zeitraum wählen.');const days=Math.min(366,Math.floor((end-start)/86400000)+1);const deposit=Math.round(days*12175.35+(days%7)*420);const payout=Math.round(deposit*(.63+Math.min(days,30)*.001));state.customFrom=from;state.customTo=to;state.customDashboard={deposit,payout,profit:deposit-payout,open:30+days*4};state.dashboardPeriod='custom';state.customRangeOpen=false;render();toast(`${days} Tage ausgewertet.`)});

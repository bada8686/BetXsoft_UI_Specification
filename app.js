...
const couponTypes=['LIVE','PREMATCH','LIVE','LIVE','PREMATCH'];
...
function couponDetailView(){
  ...
  const winningProfit=String(status).toUpperCase()==='WON'?maxProfit:'0,00';
  const bets=[
  ...
  return `<button class="back-link" data-go="coupons">← Zurück zu Wettscheinen</button><div class="ticket-head"><div><h1 style="margin:0;font-size:25px">Wettschein #${ticket}</h1><p class="muted" style="margin:5px 0 0">11.08.2026 20:41:28</p></div><span class="avatar">♙</span><strong>(${id}) ${esc(customer)}</strong><button class="icon-button" data-export title="Drucken">▣</button></div><section class="card" style="margin-top:20px"><div class="stats-row"><div class="stat"><small>Ticket Amount</small><strong class="positive">${stake}</strong></div><div class="stat"><small>Max Profit</small><strong style="color:var(--blue)">${maxProfit}</strong></div><div class="stat"><small>Winning Profit</small><strong style="color:var(--blue)">${winningProfit}</strong></div><div class="stat"><small>Status</small><span class="status ${statusClass}">${status}</span></div></div></section><section class="card table-card" style="margin-top:14px"><div class="card-pad" style="padding-bottom:10px"><h2 class="section-title" style="margin:0">▤ Wett-Details (5)</h2></div><div class="table-wrap"><table class="data-table"><thead><tr><th>#</th><th>Teams</th><th>Liga / Land</th><th>Markt</th><th>Wette</th><th>Quote</th><th>Score</th><th>Status</th></tr></thead><tbody>${bets.map(r=>`<tr>${r.map((c,i)=>`<td>${i===7?`<span class="status ${c.toLowerCase()}">${c}</span>`:c}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section><section class="ticket-meta"><div><small>Status</small><strong class="${String(status).toUpperCase()==='WON'?'positive':String(status).toUpperCase()==='OPEN'?'':'negative'}">${status}</strong></div><div><small>Winning Profit</small><strong>${winningProfit}</strong></div><div><small>Erstellt</small><strong>11.08.2026 20:41:28</strong></div><div><small>Wett-Typ</small><strong style="color:var(--blue)">LIVE</strong></div><div><small>IP-Adresse</small><strong>194.230.160.96</strong></div><div><small>Ergebniszeit</small><strong>11.08.2026 21:31:51</strong></div><div><small>Coupon Type</small><strong>KOMBINATION (5)</strong></div><div><small>Dauer</small><strong>COMPLETED</strong></div></section>`;
}
